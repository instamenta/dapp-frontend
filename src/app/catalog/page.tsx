	...
	return (
		<>
			...
			<aside id="separator-sidebar" aria-label="Sidebar"
				...
				<div className="h-full px-3 py-4 overflow-y-auto bg-gradient-to-b from-gray-800 to-transparent">
			...
			<div className="p-4 sm:ml-64">
				<div className="p-4 border-2 border-dashed rounded-lg border-gray-600 gap-10">
					{data.map((thread, index) => (
						<div key={index} className="p-4 border-2 border-dashed border-gray-700 rounded-lg mb-4 h-52">
							<div className="flex flex-row">
								<div>
									<h3 className="text-xl font-bold leading-8">{thread.name}</h3>
									<span
										className="break-words font-thin">{[thread.content, thread.content, thread.content, thread.content, thread.content, thread.content]
										.map(text => (text))}</span>
								</div>
								<img className="h-40 max-w-2xl object-center object-cover  rounded-lg aspect-video"
